...
func (e *Device) Json(messageType string) string {
	tempStruct := struct {
		*Device
	...
	jsonString, _ := json.Marshal(tempStruct)
...
func (e *Event) ParseEvent(event_name, address string, prop dbus.Variant) {
	...
	if e.Category == "Track" {
		track := prop.Value().(map[string]dbus.Variant)
		...
		tmp := struct {
			...
			Duration: track["Duration"].Value().(uint32),
			NTracks:  track["NumberOfTracks"].Value().(uint32),
			Title:    track["Title"].String(),
			TrackN:   track["TrackNumber"].Value().(uint32),
		}
		...
		e.ValueType = reflect.TypeOf(tmp)
		e.Value = string(tmpJson)

	}

}

func (e *Event) Json(messageType string) string {
	tempStruct := struct {
		*Event
		MessageType string `json:"MessageType"`
	}{
